...
class  Equipe extends DBMapper
{
    ...
    static function createTwoEquipeForBD ( $id_user )
    {
        ...
        $requete = "SELECT max(id_equipe) FROM equipe";
        try {
        ...
        // Ici on détermine l'identifiant equipe max +1 afin d'en créer un nouveau
        $resultat  = $reponse->fetch ();
        $id_equipe = $resultat[ 0 ] + 1;
        //ICI on créer l'equipe 1 dans la base de donnée
        $requete = "INSERT INTO equipe VALUES(:id_equipe, :id_user)";
        try {
            $reponse = self::$database->prepare ( $requete );
            $reponse->execute ( array ( 'id_equipe' => $id_equipe,
                                        'id_user'   => $id_user
                                ) );
        } catch ( PDOException $e ) {
            echo 'Échec lors de la connexion : ' . $e->getMessage ();
        }
        //ICI on créer l'equipe 2 dans la base de donnée
        $requete = "INSERT INTO equipe VALUES(:id_equipe, :id_user)";
        try {
            $reponse = self::$database->prepare ( $requete );
            $reponse->execute ( array ( 'id_equipe' => $id_equipe + 1,
                                        'id_user'   => $id_user
                                ) );
        } catch ( PDOException $e ) {
            echo 'Échec lors de la connexion : ' . $e->getMessage ();
    ...
    function retourneAffichageEquipeAvecAttaques ()
    {
        ...
        foreach ( $this->_personnages as $personnage ) {
            $text = $text . $personnage->retourneAffichagePersonnageAvecAttaque ();
        }
        $text = $text . '</div>';
