...
jQuery(document).ready(function () {
    jQuery("#featured").not('.fadeslider, .newsslider').superaccordion({
        slides: '.featured',
        autorotation: true,
        autorotationSpeed: 6,
        animationSpeed: 950,
...
(function ($) {
    $.fn.super_image_preloader = function (options) {
        var defaults = {repeatedCheck: 550, fadeInSpeed: 1100, delay: 610, callback: ''};
        var options = $.extend(defaults, options);
        return this.each(function () {
            var imageContainer = jQuery(this), images = imageContainer.find('img').css({
                opacity: 0,
                visibility: 'hidden'
...
(function ($) {
    $.fn.super_fade_slider = function (options) {
        ...
        var options = $.extend(defaults, options);
        return this.each(function () {
            var slideWrapper = $(this), slides = slideWrapper.find(options.slides).css({
                display: 'none',
                zIndex: 0
            }), slideCount = slides.length, currentSlideNumber = 0, interval, current_class = 'active_item', controlls = '', skipSwitch = true;
            ...
            slideWrapper.methods = {
                init: function () {
                    ...
                    if (slideCount <= 1) {
                        ...
                        slideWrapper.super_image_preloader({callback: slideWrapper.methods.preloadingDone, delay: 200});
                    ...
                    if (!skipSwitch && noAction == false) {
                        skipSwitch = true;
                        var currentSlide = slides.filter(':visible'), nextSlide = slides.filter(':eq(' + currentSlideNumber + ')');
                        if (options.appendControlls) {
...
(function ($) {
    $.fn.superaccordion = function (options) {
        ...
        return this.each(function () {
            ...
            expand_slide = slides.width(), minimized_slide = (slideWrapper.width() - expand_slide) / (slide_count - 1), overlay_modifier = 200 * (1 - options.imageShadowStrength), excerptWrapper = slideWrapper.find('.feature_excerpt'), interval = '', current_slide = 0;
            ...
            slides.each(function (i) {
                var this_slide = $(this), this_slide_a = this_slide.find('a'), real_excerpt = this_slide.find('.position_excerpt'), real_excerpt_height = real_excerpt.height(), slide_heading = this_slide.find('.sliderheading'), cloned_heading = slide_heading.clone().appendTo(this_slide_a).addClass('heading_clone').css({
            ...
            jQuery('#featured').super_image_preloader({callback: add_functionality});
            function add_functionality() {
                ...
                slides.each(function (i) {
                    var this_slide = $(this), real_excerpt = this_slide.find('.position_excerpt'), cloned_heading = this_slide.find('.heading_clone');
                    this_slide.bind(options.event, function (event, continue_autoslide) {
                        ...
                        slides.each(function (j) {
                            if (i !== j) {
                                var this_slide = $(this), real_excerpt = this_slide.find('.position_excerpt'), cloned_heading = this_slide.find('.heading_clone'), objData = this_slide.data('data'), new_pos = objData.pos_active_higher;
                                if (i < j) {
